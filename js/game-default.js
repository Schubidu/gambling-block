...
//			this.focus();
        ...
        var setInputs = function () {
            ...
            tds.each(function (i) {
                $(this).text('');
                $(this).append($('<input type="text" pattern="\\d{1,5}"  maxlength="' + maxLength + '" data-player="' + i + '" name="player-' + i + '" />'));
                if (i == 0) {
                    $(this).focus();
                }
                if (i == tds.length - 1) {
        ...
        var create = function (parent, obj) {
            ...
            nt += '</tr>' +
                '</tfoot></table>' +
                '<div class="pure-controls"><button class="pure-button button-success" type="sumbit">New Game</button></div></form>';
